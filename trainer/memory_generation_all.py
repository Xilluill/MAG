...
from omegaconf import OmegaConf, base
import torch
import wandb
...
from model.memory_generation_model import memory_generation_model,block_cache_long_model
...
class Trainer:
    def __init__(
        ...
        self.config = config
        self.num_chunks = config.get("num_chunks", 1)
    ...
    def save_checkpoint(self, save_directory):
        ...
        # save_fsdp_model(self.model, save_directory)
        # for i, model in enumerate(self.accelerator._models):
            # if self.distributed_type == DistributedType.FSDP:
            # logger.info("Saving FSDP model")
            # save_fsdp_model(self.accelerator.state.fsdp_plugin, self.accelerator, model, save_directory, i)
    ...
    def init_state(self):
        ...
        #     num_chunks = 1  # torch.randint(2, 13, (1,), device=self.device)
        # else:
        num_chunks = self.num_chunks  # torch.randint(2, 13, (1,), device=self.device)
        assert num_chunks > 1
        # dist.broadcast(num_chunks, src=0)
        # num_chunks = num_chunks.item()
        num_frames = 21 + (num_chunks - 1) * 18
        image_or_video_shape[1] = num_frames

        self.model.init_state(
            image_or_video_shape, text_prompts, negative_prompts, num_chunks
        )
    ...
    def train(self, max_train_steps):
        ...
        while self.global_step < max_train_steps:
            ...
            if self.generate_finished():
                self.init_state()

            TRAIN_GENERATOR = (
                self.global_step % (1 + self.config.dfake_gen_update_ratio) == 0
            )
            ...
            with self.accelerator.accumulate(self.model): 
                if TRAIN_GENERATOR:
                    ...
                    dmd_loss, _, dmd_grad = self.fwdbwd_one_step(train_generator=True)
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        # self.accelerator.print(f'dmd_gather_shape:{total_dmd_loss.shape}')
                        total_dmd_loss = self.accelerator.reduce(
                            dmd_loss.detach(), reduction="mean"
                        )
                        # dmd_grad = dmd_grad.mean()
                        total_dmd_grad = self.accelerator.reduce(
                            dmd_grad, reduction="mean"
                        )
                        total_generator_grad_norm = self.accelerator.reduce(
                            generator_grad_norm, reduction="mean"
                        )
                        # total_generator_grad_norm = self.accelerator.gather_for_metrics(generator_grad_norm.unsqueeze(0))
                        # total_generator_grad_norm = total_generator_grad_norm.mean()

                else:
                    critic_loss = self.fwdbwd_one_step(train_generator=False)
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        total_critic_loss = self.accelerator.reduce(
                            critic_loss.detach(), reduction="mean"
                        )
                        total_fake_score_grad_norm = self.accelerator.reduce(
                            fake_score_grad_norm, reduction="mean"
                        )
                        # total_fake_score_grad_norm = self.accelerator.gather_for_metrics(fake_score_grad_norm.unsqueeze(0))
                        # total_fake_score_grad_norm = total_fake_score_grad_norm.mean()
            ...
            if self.accelerator.sync_gradients and self.accelerator.is_main_process:
                ...
                if TRAIN_GENERATOR:
                    self.accelerator.print(
                        f"Step {self.global_step}, dmd_loss: {total_dmd_loss.item()},Grad Norm: {total_generator_grad_norm.item()}"
                    )
                    wandb_loss_dict.update(
                        {
                            "generator_loss": total_dmd_loss.item(),
                            "generator_grad_norm": total_generator_grad_norm.item(),
                            "dmdtrain_gradient_norm": total_dmd_grad.item(),
                            "step": self.global_step,
                        }
                    )
                else:
                    self.accelerator.print(
                        f"Step {self.global_step}, critic_loss: {total_critic_loss.item()}, Grad Norm: {total_fake_score_grad_norm.item()}"
                    )
                    wandb_loss_dict.update(
                        {
                            "critic_loss": total_critic_loss.item(),
                            "critic_grad_norm": total_fake_score_grad_norm.item(),
                            "step": self.global_step,
                        }
                    )
...
def main():
    ...
    parser.add_argument(
        "--config_path",
        type=str,
        default="configs/mag.yaml",
        help="Path to the configuration file",
    )
    parser.add_argument(
        "--logdir",
        type=str,
        default="logs/mg_all_debug",
    ...
    if args.ckpt_path:
        ...
        dataset = TextDataset(
            config.data_path, start_index=config.dataset_start_step * config.total_batch_size
        )
    accelerator.print(
    ...
    if model.__class__ == memory_generation_model:

        model.memory_model = fsdp_wrap(
            model.memory_model,
