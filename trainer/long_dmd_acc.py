...
class Trainer:
    ...
    def fwdbwd_one_step(self, train_generator):
        ...
        if train_generator:
            dmd_loss, mf_loss, generator_log_dict = self.model.dmd_loss()
    ...
    def init_state(self):
        ...
        self.model.init_state(image_or_video_shape, text_prompts, negative_prompts,num_chunks)
        ...
        while self.global_step < max_train_steps:
            ...
            with self.accelerator.accumulate(self.model):
                if TRAIN_GENERATOR:
                    ...
                    # total_dmd_loss = self.accelerator.gather_for_metrics(dmd_loss.detach().unsqueeze(0))
                    # dmd_grad = self.accelerator.gather_for_metrics(dmd_grad.detach().unsqueeze(0))
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        total_dmd_loss = self.accelerator.reduce(dmd_loss.detach(),reduction='mean')
                        # dmd_grad = dmd_grad.mean()
                        total_dmd_grad = self.accelerator.reduce(dmd_grad,reduction='mean')
                        total_generator_grad_norm = self.accelerator.reduce(generator_grad_norm,reduction='mean')
                    ...
                    if self.accelerator.sync_gradients:
                        ...
                        total_critic_loss =  self.accelerator.reduce(critic_loss.detach(),reduction='mean')
                        total_fake_score_grad_norm = self.accelerator.reduce(fake_score_grad_norm,reduction='mean')
            ...
            if self.accelerator.sync_gradients:
                if self.global_step % 100 == 0 and self.global_step > self.start_step:
...
def main():
    ...
    if args.ckpt_path:
        ...
        dataset = TextDataset(config.data_path,start_index=300 * config.total_batch_size)
    accelerator.print(
