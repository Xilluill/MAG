...
class WanVAEWrapper(torch.nn.Module):
    ...
    def decode_to_pixel(
        self, latent: torch.Tensor, use_cache: bool = False, offload_cpu: bool = False
    ) -> torch.Tensor:
        # from [batch_size, num_frames, num_channels, height, width]
        # to [batch_size, num_channels, num_frames, height, width]
        ...
        for u in zs:
            if offload_cpu:
                output.append(
                    decode_function(u.unsqueeze(0), scale).float().clamp_(-1, 1).squeeze(0).cpu()
                )
            else:
                output.append(
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                model_path, local_attn_size=local_attn_size, sink_size=sink_size,num_frame_per_block=num_frame_per_block,is_inference_mode=True
            )
        else:
        ...
        self.seq_len = 327600  # [1, 21, 16, 60, 104]
        self.post_init()
        ...
        if kv_cache is not None:
            ...
            if clean_x is not None:
                ...
                if classify_mode:
                    ...
                    flow_pred = self.model(  # 走这里
                        noisy_image_or_video.permute(
                            0, 2, 1, 3, 4
                        ),  # [1, 21, 16, 60, 104] -> [1, 16, 21, 60, 104]
                        t=input_timestep,
                        context=prompt_embeds,
                        seq_len=self.seq_len,
                    ).permute(0, 2, 1, 3, 4)
